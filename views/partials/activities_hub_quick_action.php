...
$activities_hub_single_id = $activities_hub_count === 1 ? (int) ($activities_hub_events[0]['id'] ?? 0) : 0;
$activities_hub_single_url = $activities_hub_single_id > 0
    ? (BASE_URL . '/event_activities.php?id=' . $activities_hub_single_id)
    : '';
$activities_hub_btn_classes = $activities_hub_sa_style
    ? trim($activities_hub_btn_class)
    : trim('action-btn ' . $activities_hub_btn_class);
?>
<?php if ($activities_hub_single_url !== ''): ?>
<a class="<?= htmlspecialchars($activities_hub_btn_classes) ?><?= $activities_hub_sa_style ? '' : ' text-decoration-none text-reset' ?>" href="<?= htmlspecialchars($activities_hub_single_url) ?>">
<?php else: ?>
<button type="button" class="<?= htmlspecialchars($activities_hub_btn_classes) ?>" data-bs-toggle="modal" data-bs-target="#activitiesHubPickModal">
<?php endif; ?>
