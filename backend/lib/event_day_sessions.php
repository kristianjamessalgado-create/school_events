...
function eventify_register_session_rsvp(mysqli $conn, int $sessionId, int $userId): array
{
    ...
    $stmt = $conn->prepare(
        'SELECT s.id, s.event_id, s.title, s.status, s.max_capacity, s.schedule_date, s.start_time, s.end_time,
    ...
    $reg = $conn->prepare('SELECT id FROM registrations WHERE user_id = ? AND event_id = ? LIMIT 1');
    if (!$reg) {
        return ['ok' => false, 'error' => 'Server error.'];
    }
    $reg->bind_param('ii', $userId, $eventId);
    $reg->execute();
    $hasReg = (bool) $reg->get_result()->fetch_assoc();
    $reg->close();
    if (!$hasReg) {
        return ['ok' => false, 'error' => 'Register for the main event first, then RSVP for this activity.'];
    }
...
function eventify_load_session_by_checkin_token(mysqli $conn, string $token): ?array
{
    ...
    $stmt = $conn->prepare("SELECT {$colList}, e.title AS event_title, e.status AS event_status, e.organizer_id, e.latitude AS event_latitude, e.longitude AS event_longitude FROM event_day_sessions s JOIN events e ON e.id = s.event_id WHERE s.checkin_token = ? LIMIT 1");
    if (!$stmt) {
        return null;
    }
...
/** Whether QR / manual activity check-in is open (only on the activity day, within its time window). */
function eventify_session_allows_checkin(array $session, ?DateTimeInterface $now = null): bool
{
    return eventify_session_is_live_now($session, null, $now);
}
